...
void FLASH_Unlock(void);                        //写入键值解锁FPEC
void FLASH_Lock(void);                          //上锁
FLASH_Status FLASH_ErasePage(uint32_t Page_Address); //页擦除
FLASH_Status FLASH_EraseAllPages(void);             //全部擦除
FLASH_Status FLASH_EraseOptionBytes(void);          //擦除选项字节，后再往RDP中写入0xA5
...
FLASH_Status FLASH_ProgramOptionByteData(uint32_t Address, uint8_t Data); //写选项字节
FLASH_Status FLASH_EnableWriteProtection(uint32_t FLASH_Pages);          //使能写保护
FLASH_Status FLASH_ReadOutProtection(FunctionalState NewState);          //启用或禁用读出保护
FLASH_Status FLASH_UserOptionByteConfig(uint16_t OB_IWDG, uint16_t OB_STOP, uint16_t OB_STDBY);   //选项字节配置，硬件软件看门狗；进入停止产生复位；进入待机产生复位
uint32_t FLASH_GetUserOptionByte(void);                                    //获取USER字节
uint32_t FLASH_GetWriteProtectionOptionByte(void);                        //读RDP，写保护字节
FlagStatus FLASH_GetReadOutProtectionStatus(void);                        //获取写保护状态
FlagStatus FLASH_GetPrefetchBufferStatus(void);                         //flash缓冲预取指 使能状态获取
void FLASH_ITConfig(uint32_t FLASH_IT, FunctionalState NewState);       //flash中断配置
FlagStatus FLASH_GetFlagStatus(uint32_t FLASH_FLAG);                    //获取状态标志位
void FLASH_ClearFlag(uint32_t FLASH_FLAG);                              //
FLASH_Status FLASH_GetStatus(void);                                    //获取当前状态
FLASH_Status FLASH_WaitForLastOperation(uint32_t Timeout);            //等待上次任务完成
