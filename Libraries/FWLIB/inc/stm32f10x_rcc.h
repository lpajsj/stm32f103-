...
void RCC_HSEConfig(uint32_t RCC_HSE);     //HSE时钟配置，旁路可能是有源时钟的配置
ErrorStatus RCC_WaitForHSEStartUp(void);  //等待HSE时钟就绪
void RCC_AdjustHSICalibrationValue(uint8_t HSICalibrationValue);  //调整HSI校准值
void RCC_HSICmd(FunctionalState NewState);                        //HSI使能
void RCC_PLLConfig(uint32_t RCC_PLLSource, uint32_t RCC_PLLMul);  //锁相环时钟来源倍频设置
void RCC_PLLCmd(FunctionalState NewState);                        //锁相环时钟使能
...
void RCC_SYSCLKConfig(uint32_t RCC_SYSCLKSource);   //系统时钟来源配置
uint8_t RCC_GetSYSCLKSource(void);                  //获取当前系统时钟来源
void RCC_HCLKConfig(uint32_t RCC_SYSCLK);           //HCLK分频配置
void RCC_PCLK1Config(uint32_t RCC_HCLK);            //APB1时钟配置
void RCC_PCLK2Config(uint32_t RCC_HCLK);            //APB2时钟配置
void RCC_ITConfig(uint8_t RCC_IT, FunctionalState NewState);  //RCC各中断使能
...
void RCC_ADCCLKConfig(uint32_t RCC_PCLK2);           //ADC时钟分频因子配置
...
void RCC_LSEConfig(uint8_t RCC_LSE);             //LSE时钟配置 旁路
void RCC_LSICmd(FunctionalState NewState);       //LSI时钟使能
void RCC_RTCCLKConfig(uint32_t RCC_RTCCLKSource);   //RTC时钟来源
void RCC_RTCCLKCmd(FunctionalState NewState);       //RTC时钟使能
void RCC_GetClocksFreq(RCC_ClocksTypeDef* RCC_Clocks);  //获取各个时钟分频 系统 总线 adc
void RCC_AHBPeriphClockCmd(uint32_t RCC_AHBPeriph, FunctionalState NewState);//外设时钟开启
...
void RCC_APB2PeriphResetCmd(uint32_t RCC_APB2Periph, FunctionalState NewState);  //外设复位
void RCC_APB1PeriphResetCmd(uint32_t RCC_APB1Periph, FunctionalState NewState);
void RCC_BackupResetCmd(FunctionalState NewState);     //备份寄存器复位
void RCC_ClockSecuritySystemCmd(FunctionalState NewState); //启用或禁用时钟安全系统
void RCC_MCOConfig(uint8_t RCC_MCO);                //选择时钟在MCO引脚输出
